...
var common = {
  on: function on(event, target, func) {
    if (target == document) $(target).on(event, func);else $(document).on(event, target, func);
  }
};
/* 이벤트 리스트 */
...
Number.prototype.strpad = function () {
  var val = this;
  if (val > 9) return val;else return '0' + val;
};
/**
